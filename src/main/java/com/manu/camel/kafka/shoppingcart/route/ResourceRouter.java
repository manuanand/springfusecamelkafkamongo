package com.manu.camel.kafka.shoppingcart.route;
...
import com.manu.camel.kafka.shoppingcart.document.Product;
...
public class ResourceRouter extends RouteBuilder {
    ...
    public void configure() throws Exception {
		...
		/*
		 * //get data back from Consumer rest("/getData") .post("showProduct")
		 * .consumes("application/json") .produces("application/json")
		 * .to("direct:fetch-from-kafka");
		 * 
		 * from("direct:fetch-from-kafka") .routeId("fetch-from-kafka")
		 * .log(LoggingLevel.INFO, "fetching data from Kafka") .marshal()
		 * .json(JsonLibrary.Jackson)
		 * .to("kafka:ProductDetails?brokers={{kafka.broker}}") .log(LoggingLevel.INFO,
		 * " Successfully fetched from kafka topic ") .setBody(simple(SUCCESS_MSG))
		 * .end();
		 * 
		 * from(
		 * "kafka:ProductDetails?brokers={{kafka.broker}}&autoOffsetReset=earliest&groupId=product&maxPollRecords=3&consumersCount=1&seekTo=")
		 * .routeId("consumerRouteRev") .log(LoggingLevel.INFO,
		 * " Retrieving data from topic : ${header[kafka.TOPIC]} , Body  :  ${in.body} "
		 * ) .to(
		 * "mongodb:mongoClient?database={{spring.data.mongodb.database}}&collection={{collection}}&operation=select")
		 * .log(LoggingLevel.INFO, "Successfully fetched from Database");
		 */
